        ...
        try {
            ...
            if (userCorreo != null && SecurityContextHolder.getContext().getAuthentication() == null) {
                ...
                if (jwtService.isTokenValid(jwt, userDetails)) {
                    // Extraer claims para manejar roles y permisos
                    Map<String, Object> claims = jwtService.extractAllClaims(jwt);
                    Collection<SimpleGrantedAuthority> authorities = new ArrayList<>();

                    // Verificar el rol principal y agregar autoridades
                    String rol = (String) claims.get("rol");
                    if (rol != null) {
                        authorities.add(new SimpleGrantedAuthority(rol));
                        log.debug("Rol extraído del token: {}", rol);

                        // Si es admin, agregar roles adicionales
                    ...
                    // Si no se extraen autoridades del token, usar las de userDetails
                    ...
                    log.info("Autenticación exitosa para: {}", userCorreo);
                } else {
